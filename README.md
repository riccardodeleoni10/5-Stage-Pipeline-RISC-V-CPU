...
Benvenuto nel repository di **riscv_pipe_cpu**, un core di processore RISC-V a 5 stadi interamente sviluppato in VHDL. L'architettura implementa tecniche avanzate di esecuzione speculativa tramite un predittore di salto bimodale a 2 bit (*Branch Target Buffer*), ottimizzando significativamente l'efficienza di esecuzione (IPC) ed eliminando i colli di bottiglia strutturali delle pipeline classiche.
...
* **XOR Speculative Masking:** Logica brevettata di rilevamento degli errori di predizione integrata nello stadio di Execute, protetta attivamente dal segnale `ex_branch` per neutralizzare falsi positivi derivanti da fenomeni di *aliasing* degli indirizzi nel BTB.
